...
subjects.forEach(subj => {
  ...
  // 1. Insert Sidebar Label
  const navItem = `<li><a href="#${subj.sidebarId}">${subj.sidebarLabel}</a></li>\n        `;
  if (!html.includes(`href="#${subj.sidebarId}"`)) {
    // Find the end of the sidebar list. It ends before a div containing a link to the quiz
    html = html.replace(/(<\/ul>\s*<div[^>]*><a[^>]*>.*?題庫<\/a><\/div>)/is, navItem + '$1');
  }

  // 2. Insert Content
  if (!html.includes(`id="${subj.sidebarId}"`)) {
    html = html.replace(/(<\/main>)/i, subj.content + '\n      $1');
  }

  fs.writeFileSync(filePath, html, 'utf8');
  console.log(`Deepened content successfully added to ${subj.file}`);
});
